...
func TestConvertFrom(t *testing.T) {
	...
	for i := 0; i < itemsPerEntry; i++ {
		for _, prefix := range []string{"tlog", "ctlog"} {
			...
			switch prefix {
			case "tlog":
				source.TLog = append(source.TLog, entry)
			case "ctlog":
				source.CTLog = append(source.CTLog, entry)
			default:
				panic("invalid type")
			}
		}
	...
	for i := 0; i < itemsPerEntry; i++ {
		for _, prefix := range []string{"fulcio", "tsa"} {
			...
			switch prefix {
			case "fulcio":
				source.CertificateAuthority = append(source.CertificateAuthority, entry)
			case "tsa":
	...
	sk := skMap["test-entry"]
	if len(sk.TLog) != 2 {
		t.Errorf("Not enough TLog entries, want 2 got %d", len(sk.TLog))
	}
	if len(sk.CTLog) != 2 {
		t.Errorf("Not enough CTLog entries, want 2 got %d", len(sk.CTLog))
	}
	if len(sk.CertificateAuthority) != 2 {
		t.Errorf("Not enough CertificateAuthority entries, want 2 got %d", len(sk.CertificateAuthority))
	}
	...
	for i := 0; i < itemsPerEntry; i++ {
		for _, prefix := range []string{"fulcio", "tsa"} {
			...
			switch prefix {
			case "fulcio":
				entry = sk.CertificateAuthority[i]
			case "tsa":
