...
// cosignVulnStatement is the pre-cosign-v3.0.6 shape of
// attestation.CosignVulnStatement. cosign v3.0.6 changed the Type field's
// JSON tag from `_type` to `type,omitempty`, silently dropping the `_type`
// key from the payload handed to policies. Policies match on `_type`, so
// keep the in-toto spec shape.
type cosignVulnStatement struct {
	in_toto.StatementHeader
	Predicate attestation.CosignVulnPredicate `json:"predicate"`
}
...
func AttestationToPayloadJSON(_ context.Context, predicateType string, verifiedAttestation PayloadProvider) ([]byte, string, error) {
	...
	case options.PredicateVuln:
		var vulnStatement cosignVulnStatement
